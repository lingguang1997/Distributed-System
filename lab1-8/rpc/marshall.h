...
enum {
	...
	RPC_HEADER_SZ = MAXX(sizeof(req_header), sizeof(reply_header)) + sizeof(rpc_sz_t) + sizeof(rpc_checksum_t)
#else
		RPC_HEADER_SZ = MAXX(sizeof(req_header), sizeof(reply_header)) + sizeof(rpc_sz_t)
#endif
};
