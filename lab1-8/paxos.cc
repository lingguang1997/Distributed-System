...
proposer::proposer(class paxos_change *_cfg, class acceptor *_acceptor, 
  ...
  : cfg(_cfg), acc (_acceptor), me (_me), break1 (false), break2 (false), 
    stable (true)
{
  cout << "[proposer]: enter, my_n.m=" << _me.c_str() << endl;
  my_n.m = _me;
  assert (pthread_mutex_init(&pxs_mutex, NULL) == 0);
  cout << "[proposer]: exit" << endl;
    ...
    if (h.get_rpcc()) {
      ...
      if (ret == paxos_protocol::OK) {
        if (resp.oldinstance == 0) {
          ...
          cout << "[prepare]: old instance, commit" << endl;
...
{
  cout << "[acceptor]: enter, is_first=" << _first << endl;
  assert (pthread_mutex_init(&pxs_mutex, NULL) == 0);
  ...
  l = new log (this, me);
  cout << "[acceptor]: instance_h=" << instance_h << endl;
